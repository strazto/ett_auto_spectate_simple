...
def is_menu_open(res_config: ResolutionConfig, template_path: str = "templates_1080p/power_menu_icon.jpg") -> bool:
...
def print_mouse(res_config: ResolutionConfig | None = None):
    ...
    if res_config:
        ...
        if (res_config.offset_x <= mouse_position[0] < res_config.offset_x + res_config.res_x and
            res_config.offset_y <= mouse_position[1] < res_config.offset_y + res_config.res_y):
            ...
            print(f"Mouse: {norm_x:.4f}, {norm_y:.4f} (Norm) | Menu: {menu_status} | Pos: {mouse_position[0]}, {mouse_position[1]}", end='')
        else:
            print(f"Mouse: OUTSIDE | Pos: {mouse_position[0]}, {mouse_position[1]}", end='')
    else:
